...
As mentioned we have instability in the logistic regression, therefore we use the reduction of **Firth**. The results are show below and are sligthly better than with tree. 

```{r first_method, echo=FALSE, warning=FALSE}
hot_firth <- brglm( opportunity  ~ .,  data = hot_to_process %>% select( !which(stringr::str_detect(names(hot_to_process), "catP$"))))
...
The main coefficient with impact and significance as expected the conditional probability are part of the variable with significance. 

```{r firsth_coef. echo=FALSE}
names_coef <-rownames(all_sum$coefficients)
